...
<#
	...
	4. Where possible, prepend your variables with 'dyrl_', for example $nc_var1.
	...
	7. These variables are used GLOBALLY across all the tools:
		...
		$GOBACK and $COMEBACK were used historically when scripts need to jump back and
			forth between each other without completely exiting
		$vf19_GAVIL gets set to 'true' when the logged in user is not an admin (see
            the \ncore\validation.ps1 file's "setUser" function to configure this)
		...
		$vf19_OPT1 gets set when a user appends an 's' to their module selection
			...
			my BASARA tool is used to digitally sign scripts, but with 's' 
...
while( $Global:vf19_Z -ne 'q' ){
    ...
    $Global:vf19_PAGE = 'X'   ## Controlled by the scrollPage function in display.ps1
