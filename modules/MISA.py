...
if HELP:
    w("""
    This is a simple python script to demonstrate how MACROSS calls   
    powershell and python tools interchangeably. Review the code and
    code comments in this and the other demo scripts for tips on how
    to use the MACROSS utilities.
    ...
    3. If you run the CLAUDIA demo script, it mirrors the HIKARU
        demo, but from a python script instead of powershell to
        show the syntax and usage differences.
            
    Hit ENTER to continue.""","y")
    input()
    exit()
    ...
    from valkyrie import psc,slp,getThis,availableTypes,collab,screenResults,\
    ...
    def main() -> None:
        ...
        if PROTOCULTURE:
            ...
            if CALLER:
                ...
                # MISA was called by another script (CALLER) using collab, but we're
                # going to be using the collab function again to demo jumping from
                # one script another and back. The reason you want to rewrite CALLER
                # with your script's name is so that the next script you collab
                # with can view your .valtype, .rtype and other attributes to tailor
                ...
                # Since python collab was already used to call MISA, we need to use the 
                ...
                # Here, we check to see if the CALLER is a python script. If it is not,
                # MISA will continue.
