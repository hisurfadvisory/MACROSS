...
<#
    ...
    NOTE 4:
        ...
        search targeted network shares, local hosts or root directories that would take
        too long to search through manually.
#>
...
<#
    ...
    -$ORDERS = the filename keyword to search for
...
#getThis 'Sz9OSUc='
$dyrl_KM = "K$(chr 214)NIG"  ## The "Ö" character always gets derp-encoded when editing files...

## Display help file
if( $HELP ){
    cls
    $vf19_LATTS['KONIG'].toolInfo() | %{
        Write-Host -f YELLOW $_
    }
    Write-Host -f YELLOW "
 $dyrl_KM performs an automated search based on filenames or extensions (for example,
 if you need to see all documents in a share, you can have $dyrl_KM focus on just
 ...
 If your enterprise uses roaming profiles, $dyrl_KM can attempt to search based on
...
function splashPage1b(){
    Write-Host -f YELLOW "               VB-6  $dyrl_KM MONSTER (Macross VFX-2/Macross Frontier)"
...
while( $dyrl_kon_LOOP ){
    ...
    if( $CALLER -or $dyrl_kon_COMMANDER -or $dyrl_kon_GENERAL ){
        ...
        cls
        splashPage1b
        Write-Host -f GREEN " $dyrl_kon_CLIENT is tasking KONIG to target " -NoNewLine;
    ...
    while( ! $dyrl_kon_FPATH ){
        ...
        #############
        Write-Host ''
        Write-Host -f GREEN " Adjusting $dyrl_KM's loadout selector... (you can filter by filesize here)"
        Write-Host "    1. " -NoNewLine;
    ...
    ## Iterate through matching directory names; the "top" directory does not get file-searched
    ...
    if( ($dyrl_kon_CTR -gt 0) -or ($Global:dyrl_kon_ADSC -gt 0) ){
        ...
        function collaborate(){
            ...
            '
            $vf19_LATTS.keys | %{
                if($vf19_LATTS[$_].valtype -like "*string*search*"){
                    $Script:dyrl_eli_STR = $vf19_LATTS[$_].fname
                    $choices = $true
                    Write-Host -f GREEN '      -Enter ' -NoNewline;
                    Write-Host -f YELLOW 'e' -NoNewline;
                    Write-Host -f GREEN " to have $($vf19_LATTS[$_].name) string-search these $dyrl_kon_numfiles files."
                }
                if($vf19_LATTS[$_].valtype -eq 'EDR'){
                    $Script:dyrl_eli_EDR = $vf19_LATTS[$_].fname
                    $choices = $true
                    Write-Host -f GREEN '      -Enter ' -NoNewline;
    ...
    if( $COMEBACK ){
        ...
        while( $dyrl_kon_Z -notMatch "^(y|n)" ){
            Write-Host -f GREEN "  Welcome back to $dyrl_KM! Do you want to delete " -NoNewLine; 
