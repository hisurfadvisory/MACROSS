...
function setConfig(){
    ...
    ########################################
    ##                    MOD SECTION
    ## If you're storing your .conf files on a web server, this should work fine,
    ## but if not you can run "curl.exe --verbose $vf19_CONFIG[0]" to see whether or
    ## not you are getting the file contents.
    ## $vf19_CONFIG[0] - config.conf
    ## $vf19_CONFIG[1] - launch.conf
    ## $vf19_CONFIG[2] - analyst.conf
    ########################################
    if($ecfg_ -Like "http*"){
        ## For each curl below, add a '-k' after '--silent' if your local server has self-signed certs
        curl.exe --silent "$ecfg_" > "$vf19_TMP\4h.txt"  
        $ecfg_ = gc "$vf19_TMP\4h.txt"
        if($ecfg_){
            $acfg_ = curl.exe --silent $vf19_CONFIG[2]  -Join ''
            $ccfg=($ecfg_).count
            if($(getHash "$vf19_TMP\4h.txt" sha256) -eq $(curl.exe --silent $vf19_CONFIG[1])){
                $current_config=$ecfg_[2..$ccfg_]
            }
        }
    }

    ## Get config file from local or network share
    else{
        $ccfg_=(gc $ecfg_).count
        if(Test-Path "$ecfg_"){
            if($(getHash $ecfg_ sha256) -eq $(gc $vf19_CONFIG[1])){
                $current_config=$(gc $ecfg_)[2..$ccfg_]
                $acfg_ = gc $vf19_CONFIG[2] -Join ''
            }
        }
    }
    ...
    ** REQUIRED CONFIGURATION SETUP **
                ...
                $info.Text = "INITIAL RUN: You must create a pair of config.conf and launch.conf files and place them in:`n $($vf19_CONFIG[0] -replace "\\config\.conf$")"
...
function setCC([switch]$c=$false,[switch]$b=$false){
    ...
    if($vf19_CONFIG[0] -Like "http*"){$gc=curl.exe --silent "$($vf19_CONFIG[0])"}
    else{$gc=gc $vf19_CONFIG[0]}
    $cc=$gc[1].Substring(32)
...
function setNewRole($file,$role,$array,[switch]$c=$false){
    ...
    $gj = setCC
    if($vf19_CONFIG[2] -Like "http*"){$ga=((curl.exe --silent $vf19_CONFIG[2]) -Join '')}
