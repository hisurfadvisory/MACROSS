...
function setUser($1,[switch]$c=$false,[switch]$i=$false){
    ...
    elseif($i){
        ...
        if($vf19_USERAUTH){
            ...
            #battroid -n vf19_DTOP -v "C:$env:HOMEPATH\Desktop"
...
function collab(){
    ...
    If you're calling a python script, the path to the mcdefs library will be passed
    as an arg, and the python script will need to use sys.argv[1] to load it. See the
    notes in the mcdefs.py file on how to import mcdefs into your python scripts. Once
    mcdefs has been imported, you can access the MACROSS global values like $vf19_MPOD
    or $vf19_DTOP via mc.MPOD, mc.DTOP, etc.
