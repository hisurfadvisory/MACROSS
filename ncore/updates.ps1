...
function toolCount(){
    ...
    foreach( $fc in (Get-ChildItem "$vf19_TOOLSDIR\$ext")){
        if( Get-Content $fc | Select -Index 0 | Select-String -Pattern "^#_superdimensionfortress" ){
            $Global:vf19_FILECT++
        }
    }
    foreach( $rc in (Get-ChildItem "$vf19_REPO\nmods\$ext")){
        if( Get-Content $rc | Select -Index 0 | Select-String -Pattern "^#_superdimensionfortress" ){
            $Global:vf19_REPOCT++
        }
    }
}
