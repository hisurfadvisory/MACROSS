...
## Define User Permissions... this isn't secure, just convoluted
    ...
    If using this script in an Active-Directory environ and your SOC or
    incident-response users have unique GPO names, you can use this
    check to restrict your script use to cybersecurity analysts. (It
    ...
    To use this function, determine a unique string in the SOC and/or
    incident-response GPO membership in your active directory, and
    base64-encode it. Then add one of these identifiers to the front
    of your b64 string, and append it to the opening comments of
    extras.ps1 as explained the Core_Functions_README.md:
...
<#
  ...
  '$Global:vf19_NEWWINDOW' to $true in your script. Be aware that
  the called script will NOT have access to MACROSS resources if
  called in a new window!
  ...
  globally set, but you don't want that particular item to be evaluated.
  By not passing to this function, you can call another script without
  using $PROTOCULTURE's value.

  OR

  If $PROTOCULTURE is set, but you have a script that can accept more than
  one item to evaluate, you can send it to this function to be passed along
  in addition to $PROTOCULTURE, and have your script recognize when both values
  exist.
