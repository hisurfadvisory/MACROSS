...
I have a couple of my custom scripts included in the github release --MYLENE and KONIG-- which make use of active directory in environments that use roaming profiles, and so instead of having the fileshare paths sitting in each script, I just keep them encoded in this comment block until they're needed.
...
values by removing the '@@@' delimiters. startUp then reads your three-character identifier at the front of each Base64 value, and uses those characters
as the index-key for a global array, <i>$vf19_MPOD</i>.
...
This is just one of several functions in utility.ps1 file that is available to all your scripts to hopefully make life a little easier.<br>
...
<br>
	<b>B.</b> slp() = Sleep function; provide it the number of seconds you want your script to pause<br>
	Usage:  <i> slp 2</i> &nbsp;&nbsp;## pauses your script for 2 seconds<br>
<br>
	<b>C.</b> startUp() = This is the first function to run when MACROSS loads. It sets many of the default variable values, and
	...
	&nbsp;&nbsp;$MONTY = python3 is installed<br>
	&nbsp;&nbsp;$SHARK = wireshark is installed<br>
	&nbsp;&nbsp;$MAPPER = nmap is installed<br>
<br>
<br>
	<b>D.</b> chooseMod() = This function builds the main menu screen in MACROSS. It reads the contents of the nmods folder, strips
...
<br>
	<b>E.</b> scrollPage() = If you have more than 9 scripts in your nmods folder, a second "page" will be created in chooseMods().
...
<b>II. validation.ps1</b><br>
	...
	<i>* the global $PROTOCULTURE value, which is the value all scripts look for as the IOC or element to investigate, does not get cleared
	until you do it manually from the menu</i><br>
<br>
	<b>B.</b> getThis() = This function will decode Base64 and Hexadecimal strings. Call it with your encoded string as the first param.
	...
	&nbsp;&nbsp;Usage:   <i>getThis $base64_string; write-host $vf19_READ</i><br>
	&nbsp;&nbsp;OR<br>
	&nbsp;&nbsp;Hex usage:   <i>getThis $hex_string 1; write-host $vf19_READ</i><br>
<br>
	<b>C.</b> SJW() = This function checks the user's privilege, which is determined in the setUser() function. Call it from your scripts to automatically alert MACROSS users that they may not have the required privilege to continue.<br>
	Usage:<br>
	 &nbsp;&nbsp;<i>SJW 'pass'</i>  &nbsp;&nbsp;## Notifies user some functions will not work<br>
	 &nbsp;&nbsp;<i>SJW 'deny</i>  &nbsp;&nbsp;## Notifies user they need elevated privileges, then kills the script<br>
<br>
	<b>D.</b> $vf19_M = This takes the $vf19_numchk value from MACROSS.ps1, and splits into 6 individual integers that can be used for mathing.<br>
	&nbsp;&nbsp;Example usage:  <i>$var = $vf19_M[2] + $vf19_M[0]</i><br>
<br>
	<b>E.</b> errMsg() = This function contains various error messages that can be reused across scripts. Change or add new messages however you
	need.<br>
	&nbsp;&nbsp;Usage:  <i>errMsg 3</i>  &nbsp;&nbsp;## Displays whichever message is in the third slot<br>
<br>
	<b>F.</b> setUser() = Attempts two different methods to set the logged in user as global $USR. If the system or active-directory method fails, it will default to using "whoami" and also set the global value $vf19_GAVIL, which tells all the MACROSS scripts that the user does not have elevated privilege. This way you can write checks to avoid loading different functions unnecessarily.<br>
<br>
	<b>G.</b> collab() = This is the function that allows your scripts to talk to each other. It must be called with (1) the name of the script you
...
<br>
	...
	&nbsp;&nbsp;6. the filepath to the MACROSS resources folder (you set this in utility.ps1 with the "tbl" key)<br>
	...
	<br>
	&nbsp;&nbsp;Usage: <i>collab 'Otherscript.ps1' 'Myscript' $var0</i>  ## Calls Otherscript.ps1, tells it 'Myscript' is sending $var0 for evaluation<br>
<br>
...
<i>*** To use verChk & dlNew, you must first set a central repo for your master copies, and set the location of your repo as a base64-encoded string prepended with "nre" in the utility.ps1 comment section***</i><br>
	...
	&nbsp;&nbsp;#_superdimensionfortress &nbsp;&nbsp;"This is a brief description of the tool"<br>
	&nbsp;&nbsp;#_ver &nbsp;&nbsp;1.0<br>
	&nbsp;&nbsp;#_class &nbsp;&nbsp;comma,separated,attributes,for,your,script (see the classes.ps1 file)<br>
...
<b>IV. utility.ps1</b><br>
	*** The comments section of this file contains base64-encoded values that are used to populate the $vf19_MPOD hashtable; values can be decoded and used by referencing thier index, like &nbsp;&nbsp;&nbsp;&nbsp;getThis $vf19_MPOD["nre"]<br>
	<b>A.</b> debugMacross() = 'debug' is an unlisted option in the MACROSS menu. Use it to change whether errors are output to the screen or not
	to troubleshoot scripts.<br>
	<b>B.</b> runSomething() = Pauses the MACROSS console and loads a fresh powershell instance so that the user can perform a quick powershell task; users can call this by typing 'shell' into the MACROSS menu. Typing "exit" returns the user to MACROSS.<br>
	<b>C.</b> decodeSomething() = From the MACROSS menu, the user can call this by typing 'dec' to quickly decode a Base64 or Hex string they may come across in an investigation.<br>
	<b>D.</b> getHash() = accepts a filepath and the hash method (md5 or sha256), and returns the hash for
	you.<br>
	    &nbsp;&nbsp;Usage:  &nbsp;&nbsp;$var = getHash $filepath 'md5'<br>
	<b>E.</b> getFile() = Your script can use this to open a dialog box for users to select a file. Pass an optional arguemt to apply a type-filter to the dialog, e.g. "Microsoft Excel Worksheet (.xlsx) | *.xlsx"<br>
	<b>F.</b> houseKeeping() = If your scripts generate file outputs, call this function to offers users the option to delete any or all of these files when they are no longer needed.<br>
	   &nbsp;&nbsp; Usage:  &nbsp;&nbsp;houseKeeping $filepath 'myscriptname'<br>
	<b>G.</b> cleanGBIO() = The "garbage_io" folder inside of "py_classes" uses .eod files to share info from powershell to python. This function ensures the directory gets cleaned out before and after every session. This is a temp fix until the mcdefs library can read-in powershell outputs by default.<br>
	<b>H.</b> pyCross() = This is the function your powershell scripts need to call if passing info *back to* a calling python script. Outputs are written to .eod files.
	<br><br>
	
<br>
<br>
<b>V. splashes.ps1</b><br>
	<b>A.</b> transitionSplash() = This function is purely cosmetic, and allows you to briefly throw some anime ASCII art on screen before launching a script.<br><br>
...
<b>VI. classes.ps1</b><br>
	...
	<b>A.</b> macross = A custom powershell class that tags every script in the \nmods folder with specific attributes that you MUST include on the third line of your scripts, tagged with "#_class " so that MACROSS will parse it correctly. In this example:<br>
	<br>
	&nbsp;&nbsp;&nbsp;&nbsp; <i>#_class  &nbsp;&nbsp; User,syslogs,Python3,SuzyQ,1 </i><br>
	The first value User is the .priv attribute, or the level of privilege required to run your script (will typically be User vs. Admin). Next, the "syslogs" value will be assigned to the .valtype attribute, describing what kind of data your script processes/returns, or what kind of actions it performs. The third value, Python3, is the .lang attribute while the fourth, SuzyQ, is the .author attribute. Finally, "1" is the .evalmax attribute, which is the maximum number of parameters that your script can accept for processing.<br>
	<br>
This is used for:<br>
&nbsp;&nbsp;-controlling which scripts get pushed to your analysts when you are using a master repository to centrally store MACROSS<br>
&nbsp;&nbsp;-allowing you to write functions that automatically know what scripts can accept what types of values for auto-evaluating:<br>
&nbsp;&nbsp;(for example, <i><b>$vf19_ATTS | where{$_.valtype -eq 'json'}</b></i> might be used to autoquery scripts that work with JSON files).<br>
	

<br>
<br>
<b>VII. py_classes\mcdefs.py</b><br>
	<b>A.</b> This is a python library that provides many of the same core functions used in the MACROSS powershell scripts, as well a "getDefaults" function that will rebuild MACROSS' $vf19_MPOD hashtable in python. See the mcdefs file 	comments for more details.
